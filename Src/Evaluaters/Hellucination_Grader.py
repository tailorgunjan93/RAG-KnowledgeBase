...
class GradeResults(BaseModel):
    binary_score: str = Field(
        description="Exactly 'yes' or 'no' checking for result relevance for query submitted by user."
    )
...
def result_evaluater(user_query, output_response):
    ...
    system = """
        You are an specialist RAG Agent which evaluates results relevance with user query.
        You receive:
    ...
    try:
        ...
        print(f"Hellucination_Grader error: {e}")
        return "no"
